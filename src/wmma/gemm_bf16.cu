...
__global__ void wmma_bf16_kernel(const __nv_bfloat16 *a, const __nv_bfloat16 *b, int M, int N,
                                 float *c, int K) {
...
#pragma unroll
  for (size_t i = 0; i < loop_n; ++i) {
    wmma::fragment<wmma::matrix_a, fragment_m, fragment_n, fragment_k, __nv_bfloat16, wmma::row_major> a_frag;
    wmma::fragment<wmma::matrix_b, fragment_m, fragment_n, fragment_k, __nv_bfloat16, wmma::col_major> b_frag;
...
int wmma_bf16(const float *a, const float *b, int M, int N, float *c, int K,
              ...
              BF16TensorKenerlType kenerl_type, bool need_pinned_memory) {
  if (M < 0 || N < 0 || K < 0) {
    return -1;
  }
  ...
  copy_and_cast_data_to_device(a, b, M, N, K, dev_a, dev_b, m, n, k, matrix_b_is_col_major);
  switch (kenerl_type) {
    case BF16TensorKenerlType::TensorKenerl_16_16_16:
      wmma_bf16_kernel<16,16,16><<<grid, WARP_SIZE>>>(dev_a, dev_b, m, n, dev_c,k);
      break;
    case BF16TensorKenerlType::TensorKenerl_8_32_16:
      wmma_bf16_kernel<8,32,16><<<grid, WARP_SIZE>>>(dev_a, dev_b, m, n, dev_c,k);
      break;
    case BF16TensorKenerlType::TensorKenerl_32_8_16:
      wmma_bf16_kernel<32,8,16><<<grid, WARP_SIZE>>>(dev_a, dev_b, m, n, dev_c,k);
      break;
  }
...
int wmma_bf16(const __nv_bfloat16 *a, const __nv_bfloat16 *b, int M, int N,
  ...
  copy_data_to_device(a, b, M, N, K, dev_a, dev_b, m, n, k, matrix_b_is_col_major);
  switch (kenerl_type) {
    case BF16TensorKenerlType::TensorKenerl_16_16_16:
      wmma_bf16_kernel<16,16,16><<<grid, WARP_SIZE>>>(dev_a, dev_b, m, n, dev_c,k);
      break;
